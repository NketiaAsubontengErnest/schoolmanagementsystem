...
<main class="h-full overflow-y-auto">
    <div class="container px-6 mx-auto grid">
        ...
        <?php if (Auth::access('headmaster')): ?>
            <div class="py-4">
                <a href="<?= HOME ?>/employees/add"
                    ...
                    <thead>
                        <tr
                            ...
                            <?php if (Auth::access('administrator')): ?>
                                ...
                                <th class="px-4 py-3">Make User</th>
                        ...
                        <?php if ($rows) : ?>
                            <?php foreach ($rows as $row) : ?>
                                <tr class="text-gray-700 dark:text-gray-400">
                                    <td class="px-4 py-3">
                                        <p class="font-semibold"><?= esc($row->staffid) ?></p>
                                    </td>                                    
                                    <td class="px-4 py-3">
                                    ...
                                    <?php if (Auth::access('administrator')): ?>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center space-x-4 text-sm">
                                                <a href="<?= HOME ?>/employees/edit/<?= esc($row->id) ?>"
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center space-x-4 text-sm">
                                                <?php if ($row->job_titil == 'teacher'): ?>
                                                    <a href="<?= HOME ?>/employees/assignclass/<?= esc($row->id) ?>"
                                                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                        aria-label="Edit">
                                                        <svg
                                                            class="w-5 h-5"
                                                            ...
                                                            fill="currentColor"
                                                            viewBox="0 0 20 20">
                                                            <span><?= $row->assignclass == 0 ? 'Assign' : 'Un Assign' ?></span>
                                                        </svg>
                                                    </a>
                                                <?php endif; ?>
                                            </div>
                                        ...
                                        <td class="px-4 py-3">
                                            <div class="flex items-center space-x-4 text-sm">
                                                <?php if ($row->adduser == 0): ?>
                                                    <form action="" method="post">
                                                        <button name="user" value="<?= esc($row->id) ?>"
                                                            class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                                                            aria-label="Make">
                                                            <svg
                                                                class="w-5 h-5"
                                                ...
                                                <?php else: ?>
                                                    <form action="" method="post">
                                                        <button name="unuser" value="<?= esc($row->id) ?>"
                                        ...
                                        <td>
                                            <?php if ($row->adduser != 0): ?>
                                                <form action="" method="post">
                                                    <button name="reset" value="<?= esc($row->staffid) ?>"
